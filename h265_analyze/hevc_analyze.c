...
static int nal_sum[HEVC_NAL_UNIT_TYPE_SUFFIX_SEI_NUT + 1];
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        while (find_nal_unit(p, sz, &nal_start, &nal_end) > 0)
        {
            ...
            p += nal_start;
            read_debug_hevc_nal_unit(h, p, nal_end - nal_start, &nal_type);
            if (nal_type <= HEVC_NAL_UNIT_TYPE_SUFFIX_SEI_NUT) {
                nal_sum[nal_type]++;
            }
/*
...
*/
    ...
    for(int i = HEVC_NAL_UNIT_TYPE_TRAIL_N; i <= HEVC_NAL_UNIT_TYPE_SUFFIX_SEI_NUT; i++) {
        printf("naltype:%d count:%d\n", i, nal_sum[i]);
    }
